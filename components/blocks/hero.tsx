...
export const Hero = ({ data }: { data: PageBlocksHero }) => {
  return (
    <Section background={data.background!} className='flex min-h-[calc(100svh-5rem)] flex-col items-center gap-10 lg:flex-row lg:gap-20'>
      ...
      {data.image && (
        <AnimatedGroup variants={transitionVariants} className='w-full lg:w-1/2'>
          <div className='relative max-w-full overflow-hidden' data-tina-field={tinaField(data, 'image')}>
            <div className='bg-background relative mx-auto max-w-lg overflow-hidden rounded-sm border p-2 shadow-2xl lg:max-w-none'>
...
const ImageBlock = ({ image }: { image: PageBlocksHeroImage }) => {
  ...
  if (image.videoSrc) {
    return (
      <video
        className='z-2 border-border/50 aspect-square relative h-auto max-w-full rounded-sm border object-cover'
        autoPlay
        loop
        muted
        playsInline
